...
function main(obj, cb){
    ...
    function send(text){
      ...
      transport.sendMail(settings, function(error, response){
        if (error){
          ...
          // Add to IP quota
          storage.set(quotaID, sends + 1);
          setTimeout(function(){
            var amount = storage.get(quotaID) - 1;
            storage.set(quotaID, amount);
          }, 1000 * 60 * 60 * 24);
  ...
  function remove(id){
    ...
    if (item){
      // TODO: a real job queue instead of using timeouts
